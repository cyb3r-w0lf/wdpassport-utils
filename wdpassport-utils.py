...
def handle_interrupt(signal, frame):
	print(f"\n{Fore.GREEN}[!] Program interrupted. Exiting gracefully...{Fore.RESET}")
	sys.exit(0)
...
def sec_status_to_str(security_status):
	if security_status == 0x00:
		return f"{Fore.GREEN}No lock{Fore.RESET}"
	elif security_status == 0x01:
		return f"{Fore.RED}Locked{Fore.RESET}"
	elif security_status == 0x02:
		return f"{Fore.GREEN}Unlocked{Fore.RESET}"
	elif security_status == 0x06:
		return f"{Fore.RED}Locked, unlock blocked{Fore.RESET}"
	elif security_status == 0x07:
		return f"{Fore.RED}No keys{Fore.RESET}"
	else:
		return f"unknown"
...
def cipher_id_to_str(cipher_id):
	if cipher_id == 0x10:
		return f"{Fore.MAGENTA}AES_128_ECB{Fore.RESET}"
	elif cipher_id == 0x12:
		return f"{Fore.MAGENTA}AES_128_CBC{Fore.RESET}"
	elif cipher_id == 0x18:
		return f"{Fore.MAGENTA}AES_128_XTS{Fore.RESET}"
	elif cipher_id == 0x20:
		return f"{Fore.MAGENTA}AES_256_ECB{Fore.RESET}"
	elif cipher_id == 0x22:
		return f"{Fore.MAGENTA}AES_256_CBC{Fore.RESET}"
	elif cipher_id == 0x28:
		return f"{Fore.MAGENTA}AES_256_XTS{Fore.RESET}"
	elif cipher_id == 0x30:
		return f"{Fore.MAGENTA}Full Disk Encryption{Fore.RESET}"
	else:
...
def unlock():
	...
	passwd = getpass.getpass(f"{Fore.CYAN}[wdpassport] password for {device_name}: {Fore.RESET}")
...
def main(argv): 
	...
	for disk_device in context.list_devices(subsystem='block', DEVTYPE='disk'):
		...
		# Skip devices that are of type 'cd' and 'sr' (which are typically virtual CD drives).
		if 'sr' in disk_device.device_node or 'cd' in disk_device.device_node:
			continue
	...
	if len(passport_devices) == 0:
		...
		for idx, dev in enumerate(passport_devices):
			print(f"{idx + 1}. {dev.device_node} - {dev.get('ID_MODEL', 'Unknown Model')}")
		
		try:
			device_index = int(input("\033[94m[+] Enter the number corresponding to the device you want to operate on: \033[0m")) - 1
			if device_index < 0 or device_index >= len(passport_devices):
				print(fail("Invalid device selection."))
				sys.exit(1)
			device = passport_devices[device_index]
		except ValueError:
	...
	if not args.unlock and not args.change_passwd and not args.erase and not args.mount:
		status = get_encryption_status()
		print(f"{Fore.CYAN}Device: {Fore.YELLOW}{device_name} -> {device.get('ID_MODEL')}{Fore.RESET}")
		print(f"{Fore.CYAN}Security status: {Fore.RESET}{sec_status_to_str(status["Locked"])}")
		print(f"{Fore.CYAN}Encryption type: {Fore.RESET}{cipher_id_to_str(status["Cipher"])}")
